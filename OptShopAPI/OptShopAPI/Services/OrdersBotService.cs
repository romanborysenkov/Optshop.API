         ...
         public async void SumariseDeliveryPrice(object sender, MessageEventArgs e)
         {
            ...
            if (int.TryParse(msg.Text, out deliveryPrice))
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    //  var response = await client.PutAsync($"https://optshop-antek-dev-001.azurewebsites.net/api/Payment/", fin);
                    var response = await client.PutAsync($"http://localhost:5000/api/Payment/", fin);
